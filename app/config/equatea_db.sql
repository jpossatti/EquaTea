...
--
-- Host: 127.0.0.1
-- Tempo de geração: 18/08/2026 às 22:45
...
(2, 1, 2, 3, 0, '2026-08-17 10:54:12', NULL, 2),
...
(8, 1, 3, 4, 0, '2026-08-18 11:30:49', NULL, 1),
...
(1, 'Professor Carlos Silva', 'carlos@escola.com', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'professor', '2026-08-18 13:54:10', NULL, 1),
(2, 'Professora Ana Santos', 'ana@escola.com', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'professor', '2026-08-18 13:54:10', NULL, 1),
(3, 'Ana Silva', 'ana.silva@escola.com', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'aluno', '2026-08-18 13:54:11', NULL, 1),
(4, 'Carlos Souza', 'carlos.souza@escola.com', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'aluno', '2026-08-18 13:54:11', NULL, 1),
(5, 'Mariana Costa', 'mariana.costa@escola.com', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'aluno', '2026-08-18 13:54:11', NULL, 1),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_progresso_aluno`  AS SELECT `u`.`nome` AS `aluno`, concat(`e`.`a`,'x + ',`e`.`b`,' = ',`e`.`c`) AS `equacao`, `e`.`dificuldade` AS `dificuldade`, `p`.`passo_atual` AS `passo_atual`, `p`.`concluida` AS `concluida`, `p`.`data_inicio` AS `data_inicio`, `p`.`data_conclusao` AS `data_conclusao`, `p`.`tentativas` AS `tentativas`, CASE WHEN `p`.`concluida` = 1 THEN 'Concluído' ELSE concat('Passo ',`p`.`passo_atual`,'/4') END AS `status_progresso` FROM (((`progresso_aluno` `p` join `alunos` `a` on(`p`.`aluno_id` = `a`.`id`)) join `usuarios` `u` on(`a`.`usuario_id` = `u`.`id`)) join `equacoes` `e` on(`p`.`equacao_id` = `e`.`id`)) ORDER BY `u`.`nome` ASC, `p`.`data_inicio` DESC ;
